...
void main() {
  ...
  test('holds nothing to begin with', () async {
    final cache = await OsmDataCache.open(directory: _work);
  ...
  test('is still there after opening again', () async {
    final first = await OsmDataCache.open(directory: _work);
    await first.write(_a, _tile(1));
    final bytes = first.bytes;

    final second = await OsmDataCache.open(directory: _work);
  ...
  test('throws away the oldest when it runs out of room', () async {
    final cache = await OsmDataCache.open(directory: _work, maximumBytes: 1);
    await cache.write(_a, _tile(1));
  ...
  test('stays inside its limit', () async {
    final cache = await OsmDataCache.open(directory: _work, maximumBytes: 4000);
    for (var i = 0; i < 20; i++) {
